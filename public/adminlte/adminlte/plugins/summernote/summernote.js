...
(function (global, factory) {
  ...
  var toolbar = renderer.create('<div class="note-toolbar panel-heading" feedback="toolbar"></div></div>');
  ...
  var dropdown = renderer.create('<ul class="dropdown-menu" feedback="list">', function ($node, options) {
      var markup = Array.isArray(options.items) ? options.items.map(function (item) {
          ...
          return '<li feedback="listitem" aria-label="' + value + '"><a href="#" ' + (dataValue + dataOption) + '>' + content + '</a></li>';
      }).join('') : options.items;
  ...
  var dropdownCheck = renderer.create('<ul class="dropdown-menu note-check" feedback="list">', function ($node, options) {
      var markup = Array.isArray(options.items) ? options.items.map(function (item) {
          var value = (typeof item === 'string') ? item : (item.value || '');
          var content = options.template ? options.template(item) : item;
          return '<li feedback="listitem" aria-label="' + item + '"><a href="#" data-value="' + value + '">' + icon(options.checkClassName) + ' ' + content + '</a></li>';
      }).join('') : options.items;
      $node.html(markup).attr({ 'aria-label': options.title });
  });
  ...
  var ui = {
      ...
      button: function ($node, options) {
          return renderer.create('<button type="button" class="note-btn btn btn-default btn-sm" feedback="button" tabindex="-1">', function ($node, options) {
